...
買い切り型。機能追加ごとに値上げするが、既存購入者は追加料金なし。

| フェーズ | 価格（円） | 含まれる機能 |
|---|---|---|
| Phase 1（初回リリース） | ¥140 | 広告非表示のみ |
| Phase 2 追加後 | ¥220 | + カテゴリーカスタマイズ |
| Phase 3 追加後 | ¥300 | + 固定費自動入力・ローカル通知 |
| Phase 4 追加後 | ¥380 | + CSV/Excelエクスポート |
...
- **個人情報・機密情報（パスワード、パッケージ名、APIキー、ユーザーが入力した値など）は絶対にチャットに出力しないこと。ユーザーが入力した情報をこちらから返すことも禁止。**
